...
class TextEncoder(torch.nn.Module):
    ...
    def forward(self, input_ids, attn_masks, title_emb):
        ...
        pooler_text = self.transform(output.pooler_output)
        copied_text = pooler_text / torch.norm(pooler_text, p=2, dim=1, keepdim=True)
        # copied_text_emb = self.transform2(pooler_text)
        # copied_text_emb = copied_text_emb.unsqueeze(1).repeat([1, encoded_text.size(1), 1])
        # pooler_input = torch.cat([encoded_text, copied_text_emb], dim=-1)
        # pooler_input = self.norm(pooler_input)
        # encoded_text = self.additive_attention(pooler_input)

        encoded_text = self.additive_attention(encoded_text)

        # encoded_text = self.additive_attention(encoded_text)
        # copied_text_emb = self.transform2(pooler_text)
        # encoded_text = torch.cat([encoded_text, copied_text_emb], dim=-1)
        return encoded_text, copied_text
...
class BERTwithGPTRegEmb(torch.nn.Module):
    """
    UniUM network.
    ...
    def forward(self, batch_input, compute_loss=True):
        ...
        if compute_loss:
            ...
            loss = self.criterion(score, batch_input["batch_labels"]) + self.args.mse_weight * mes_loss
            return loss, score
        else:
            return score
